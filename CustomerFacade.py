...
class CustomerFacade(FacadeBase):
    ...
    def add_ticket(self, ticket):
        ...
        if not flight:
            ...
            except NoRemainingTicketsError as e:
                print(e)
                print('Function failed, no remaining tickets for this flight.')
                return
    ...
    def remove_ticket(self, id_):
        ...
        if id_ <= 0:
            print('Function failed, id must be positive.')
            return
        if not self.repo.get_by_condition(Ticket, lambda query: query.filter(Ticket.id == id_).all()):
